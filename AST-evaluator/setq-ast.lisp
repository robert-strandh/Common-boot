...
(defun setq-iterations
    (client environment tempc tempb tempa value-asts reference-asts)
  (loop for value-ast in value-asts
        for variable-reference-ast in reference-asts
        collect
...
(defmethod cps (client environment (ast ico:setq-ast) continuation)
  (let ((value-asts (reverse (ico:value-asts ast)))
        (reference-asts (reverse (ico:variable-name-asts ast)))
        (tempc (make-symbol "C"))
        (tempb (make-symbol "B"))
        (tempa (make-symbol "A")))
