              ...
              (make-before-continuation
               (lambda (&rest ,values-temp)
                 ...
                 (let ((dynamic-environment dynamic-environment))
                   ...
                   ;; evaluation of the VALUES-AST, and we need to
                   ;; pair them up with the symbols in the list.  The
                   ;; problem here is that we should give the symbol a
                   ;; value only if it has a corresponding value in
                   ;; the list of values.  So we start with the
                   ;; symbols in SYMBOLS-TEMP that do have a
                   ;; corresponding value.
                   (loop for symbol in ,symbols-temp
                         ...
                         do (push (make-instance 'special-variable-entry
                                    ...
                                    :value value)
                                  dynamic-environment))
                   ;; At this point, if the list of values in
                   ;; VALUES-TEMP was shorter than the list of symbols
                   ;; in SYMBOLS-TEMP, then some symbols have not been
              ...
              (make-before-continuation 
               (lambda (&rest temp)
                 ...
                 ,(cps client environment
                       (ico:values-ast ast)
                       continuation-variable))
               :origin '(ico:origin (ico:values-ast ast))
